            ...
            {section.key === "heroes" && (
              <div className="grid gap-3">
                {HERO_CLASSES.map((hero) => (
                  <div
                    key={hero.name}
                    className={`border ${hero.color} rounded p-3 bg-gray-800`}
                  >
            ...
            {section.key === "equipment" && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {EQUIPMENT_SLOTS.map((eq) => (
                  <div
                    key={eq.slot}
                    className="flex items-start gap-3 p-2 bg-gray-800 rounded border border-gray-700"
                  >
            ...
            {section.key === "runes" && (
              <div className="space-y-4">
                {Object.entries(RUNE_TREES).map(([treeName, tree]) => (
                  <div
                    key={treeName}
                    className={`border ${tree.color} rounded p-3 bg-gray-800`}
                  >
            ...
            {section.key === "difficulty" && (
              <div className="space-y-3">
                {DIFFICULTY_TIERS.map((tier) => (
                  <div
                    key={tier.name}
                    className={`border ${tier.color} rounded p-3 bg-gray-800`}
                  >
